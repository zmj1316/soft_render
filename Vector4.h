...
public:
	...
	static void mul(float*v0,float*v1,float*dst)
	{
	...
	static void vecAdd(float* ss,  float* in1, float* in2,float *in3, float* out, int N){
		//one-dimensional arrays for AMP
		Concurrency::array_view<float, 1> in1V(N, in1), in2V(N, in2), in3V(N,in3),ssV(N,ss);
		Concurrency::array_view<float, 1> outV(N, out);
		outV.discard_data(); //ignore existing data as we'll overwrite it anyway
		parallel_for_each(outV.get_extent(), [=](Concurrency::index<1> i)
			restrict(amp) { //only AMP can do this
			outV[i] = ssV[0] * in1V[i] + ssV[1] * in2V[i] + ssV[2] * in3V[i];
		});
		outV.synchronize(); //tell everyone i'm done so dependent processing will wait.
		//for (size_t i = 0; i < N; i++)
		//{
		//	out[i] = ss[0] * in1[i] * ss[1] * in2[i] + ss[2] * in3[i];
		//}
	}
